(function($) {
    $.fn.Huploadify = function(opts) {
        var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-info"><span class="up_filename">${fileName}</span><span class="btn btn-default btn-sm uploadbtn">上传</span><span class="delfilebtn">&times;</span></div><div class="preview-box"></div><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div></div>';
        var defaults = {
        ...
        this.each(function() {
            ...
            var inputStr = '<input id="'+ pId +'_selectbtn" class="selectbtn" style="display:none;" type="file" name="fileselect[]"';
            ...
            var uploadFileListStr = '<div id="'+ pId +'_fileuploadqueue" class="uploadify-queue"></div>';
            ...
            fileObj = {
                ...
                funSelect:function(files) {
                    _this.find('.uploadify-queue').show();
                    for (var i = 0, len = files.length; i < len; i++) {
                        var file = files[i];
                        //处理模板中使用的变量
                        var $html = $(option.itemTemplate.replace(/\${fileID}/g, pId +"_fileuploadfile_"+ file.index).replace(/\${fileName}/g, file.name).replace(/\${fileSize}/g, formatFileSize(file.size)).replace(/\${instanceID}/g, _this.attr("id")));
                        //如果是自动上传，去掉上传按钮
                        if (option.auto) {
                        ...
                        //判断是否预览图片
                        if (option.previewImg) {
                            var $prevbox = $html.find('.preview-box');
                            if (option.previewLoadimg) $prevbox.html('<img src="'+ option.previewLoadimg +'" height="114">');
                            fileObj.previewImg(file, $prevbox);
                        } else {
                            $html.find('.preview-box').remove();
                        }
                        //判断是否显示已上传文件大小
                ...
                onProgress:function(file, loaded, total) {
                    var eleProgress = _this.find("#"+ pId +"_fileuploadfile_"+ file.index + " .uploadify-progress");
                ...
                previewImg: function(file, $obj) {
                    if (file && $obj.length) {
                        ...
                        reader.onload = function(e) {
                            var img = '<span class="img-box"><img src="' + e.target.result + '"></span>';
                            $obj.html(img);
                        };
                ...
                funGetProgressWidth:function(index) {
                    var eleProgressBar = _this.find("#"+ pId +"_fileuploadfile_"+ index + " .uploadify-progress-bar");
                ...
                funDealFiles:function(files) {
                    ...
                    for (var i = 0, len = files.length; i < len; i++) {
                        ...
                        files[i].id = pId +"_fileuploadfile_"+ files[i].index;
                    }
                    //执行选择回调
                ...
                funDeleteFile:function(index) {
                    for (var i = 0, len = this.fileFilter.length; i < len; i++) {
                        ...
                        if (file.index == index) {
                            ...
                            _this.find("#"+ pId +"_fileuploadfile_"+ index).fadeOut('normal', function() {
                                var filelen = _this.find('.uploadify-queue-item:visible').length;
                                if (filelen == 0) _this.find('.uploadify-queue').hide();
                            });
                ...
                funUploadFile:function(file) {
                    ...
                    //保存原始为切割的文件
                    var thisfile = _this.find("#"+ pId +"_fileuploadfile_"+ file.index);
                    ...
                    if (option.breakPoints) {
                        var fileName = file.name, fileId = file.id, fileIndex = file.index, fileSize = file.size;
                        //先保存原来的文件名称
            ...
            //如果允许拖动上传
            if (option.dragDrop) {
                _this[0].ondragover = function(ev) {
                    /*拖拽元素在目标元素头上移动的时候*/
                    ev.preventDefault();
                    return true;
                };
                _this[0].ondrop = function(e) { 
                    e.stopPropagation();
                    e.preventDefault();
                    fileObj.funGetFiles(e);
                };

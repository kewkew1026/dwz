...
var navTab = {
	...
	_switchTab: function(iTabIndex){
        ...
        this._getMoreLi().removeClass("active").eq(iTabIndex).addClass("active");
			...
			if (url) {
				if ($tab.hasClass("external")) {
					...
					if ($.isEmptyObject(op.data)) { //获取pagerForm参数
						var $pagerForm = $("#pagerForm", $panel);
						op.data = $pagerForm.size() > 0 ? $pagerForm.serializeJson() : {};
                        /*K'naan 添加清除多余的查询参数*/
                        if (op.clearQuery) {
                            var pageParams = {pageNum:1, numPerPage:30, orderField:'orderField', orderDirection:'orderDirection'};
                            $.each(pageParams, function(i, n) {
                                for (i in (op.data)) {
                                    if (pageParams[i]) {
                                        pageParams[i] = (op.data)[i];
                                    }
                                }
                            });
                            op.data = pageParams;
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		...
		if (iOpenIndex >= 0){
            ...
            var tabFrag = '<li tabid="#tabid#"><a href="javascript:" title="#title#"><span>#title#</span></a><span class="close">&times;</span></li>';
